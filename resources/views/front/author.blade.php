...
<body class="font-[Poppins] pb-[83px]">
  ...
  <nav id="Category" class="max-w-[1130px] mx-auto flex justify-center items-center gap-4 mt-[30px]">
	...
	<section id="author" class="max-w-[1130px] mx-auto flex  flex-col gap-[30px] mt-[70px]">
		<div id="title" class="flex items-start gap-[10px] mb-10">
			{{-- <h1 class="text-4xl leading-[45px] font-bold">Article News</h1>
			<h1 class="text-4xl leading-[45px] font-bold">by -</h1> --}}
		...
		<div id="content-cards" class="grid grid-cols-3 gap-[30px] mt-5">
      ...
      <a href="{{ route('front.details',$news->slug) }}" class="card">
				<div
					...
					<div class="flex flex-col gap-[6px]">
						<h3 class="text-lg leading-[27px] font-bold">	{{ Str::limit($news->name,57) }}</h3>
						<p class="text-sm leading-[21px] text-[#A3A6AE]">{{ $news->created_at->format('M d, Y') }}</p>
					</div>
				</div>
			</a>
      @empty
        <p>Penulis belum memiliki artikel</p>
      @endforelse
